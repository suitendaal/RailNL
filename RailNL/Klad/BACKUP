...
def CalculateScore(trajecten, criticalConnections):
    ...
    for traject in trajecten:
        ...
        for i in range(len(traject[0]) - 1):
            if [traject[0][i], traject[0][i + 1]] in criticalConnections or [traject[0][i + 1], traject[0][i]] in criticalConnections:
                if [traject[0][i], traject[0][i + 1]] not in critical_connections_past and [traject[0][i + 1], traject[0][i]] not in critical_connections_past:
                    critical_connections_past.append([traject[0][i], traject[0][i + 1]])
